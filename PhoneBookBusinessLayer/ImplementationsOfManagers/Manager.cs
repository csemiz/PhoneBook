...
namespace PhoneBookBusinessLayer.ImplementationsOfManagers
{
        ...
        where TModel : class, new()
    {
        ...
        public IDataResult<TViewModel> Add(TViewModel model)
        {
            try
            {
                //Bize parametre olarak gelen DTO'yu repoya gonderemiyoruz.Repoya modelin kendisi gonderilmelidir. bu nedenle mapper ile donusum yaptik.
                TModel tmodel = _mapper.Map<TViewModel, TModel>(model);

                int result = _repo.Add(tmodel);//tmodel repo ile veritabanına eklendi.
                                               //Tmodel'in artık id'si var.
